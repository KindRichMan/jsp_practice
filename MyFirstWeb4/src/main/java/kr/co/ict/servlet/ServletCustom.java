...
public class ServletCustom extends HttpServlet {
	 ...
	 */
	public void destroy() {
		System.out.println("/test 접속시 생성된 객체는 서버 종료로 인해 파기됩니다.");
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("/test 주소 접속 확인");
	 ...
	 //"jpa"라는 이름으로 들어온 요소를 콘솔에 찍도록 추가해주세요.
		...
		//response.sendRedirect("http://localhost:8181/MyFirstWeb/servletForm/testResult.jsp");
		...
		RequestDispatcher dp = request.getRequestDispatcher("/servletForm/testResult.jsp");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("post방식 :"+ "/test 주소 post방식으로 접속함");
		// servletForm 내부에 있던 ~~~Form.jsp의 목적지는 그대로 두고,
		// 전송방식만 post로 바궈서 데이터를 날려주시고
	    ...
	    request.setAttribute("boot",boot);
	    request.setAttribute("jpa",jpa);
	    RequestDispatcher dp = request.getRequestDispatcher("/servletForm/testPostResult.jsp");
	    dp.forward(request, response);
